...
from backend.app.schemas.cart import CartItemCreate, CartResponse, CartItemUpdate, CartItemUpdate, CartItem
from fastapi import HTTPException, status

class CartService:
    def __init__(self, db: Session):
        self.product_repositories = ProductRepositories(db)

    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        product = self.product_repositories.get_by_id(item.product_id)
        if product is None:
            raise HTTPException(
    ...
    def update_cart_item(self, cart_data: Dict[int, int], item: CartItemUpdate) -> Dict[int, int]:
        if item.product_id not in cart_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Item by id: {item.product_id} not found"
            )
        cart_data[item.product_id] = item.quantity
        return cart_data
    ...
    def remove_from_cart(self, cart_data: Dict[int, int], product_id: int) -> Dict[int, int]:
        if product_id not in cart_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Item by id: {product_id} not found on cart"
            )
    ...
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:
        ...
        products = self.product_repositories.get_multi_by_ids(product_ids)
